...
class ArticlesController extends Controller {
  ...
  async index() {
    ...
    await ctx.validate(this.queryListParamsRules, body);
    const res = await service.articles.index(body);
    ctx.helper.success({
      ctx,
      res,
    });
  ...
  async create() {
    ...
    await ctx.validate(this.createRule, body);
  ...
  async changeStatus() {
    ...
    const body = ctx.request.body;
    const id = ctx.params.id;
    ctx.validate(this.changeStatusRule, body);
    const res = await service.articles.changeStatus({ id, status: body.status });
    ctx.helper.success({
      ctx,
      res,
    });
